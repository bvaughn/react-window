...
describe('FixedSizeGrid', () => {
  let defaultProps, getScrollbarSize, itemRenderer, onItemsRendered; // Use PureComponent to test memoization.
  ...
  describe('style overrides', () => {
    ...
    it('should support style prop', () => {
      const rendered = ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} style={{
        backgroundColor: 'red'
      }} />);
  ...
  describe('overscanColumnCount and overscanRowCount', () => {
    ...
    describe('overscanCount', () => {
      ...
      it('should warn about deprecated overscanRowsCount or overscanColumnsCount prop', () => {
        ...
        expect(console.warn).toHaveBeenCalledTimes(1);
        expect(console.warn).toHaveBeenLastCalledWith('The overscanColumnsCount and overscanRowsCount props have been deprecated. ' + 'Please use the overscanColumnCount and overscanRowCount props instead.');
      ...
      it('should use overscanColumnCount and overscanRowCount if both them and deprecated props are provided', () => {
        spyOn(console, 'warn');
        ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} initialScrollLeft={100} initialScrollTop={100} overscanCount={1} overscanColumnsCount={2} overscanColumnCount={3} overscanRowsCount={2} overscanRowCount={3} />);
        expect(onItemsRendered.mock.calls).toMatchSnapshot();
      });
  ...
  describe('scrollTo method', () => {
    ...
    it('should allow only scrollLeft or scrollTop values to be specified', () => {
      ...
      expect(onItemsRendered).toHaveBeenLastCalledWith(expect.objectContaining({
  ...
  describe('scrollToItem method', () => {
    ...
    it('scroll with align = "auto" should work with partially-visible items', () => {
      const rendered = ReactTestRenderer.create( // Create list where items don't fit exactly into container.
  ...
  describe('itemKey', () => {
    ...
    it('should allow items to be moved within the collection without causing caching problems', () => {
      ...
      const keyMapItemRenderer = jest.fn(({
        index,
        style
      ...
      const itemKey = jest.fn(({
        columnIndex,
        rowIndex
      }) => keyMap[rowIndex][columnIndex]);
      const rendered = ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} columnCount={3} rowCount={2} itemKey={itemKey}>
          {ItemRenderer}
        </FixedSizeGrid>);
      expect(itemKey).toHaveBeenCalledTimes(6);
      itemKey.mockClear();
  ...
  describe('custom element types', () => {
    ...
    it('should support spreading additional, arbitrary props, e.g. id', () => {
      ...
      ReactDOM.render(<FixedSizeGrid {...defaultProps} innerElementType={forwardRef((props, ref) => <div ref={ref} id="inner" {...props} />)} outerElementType={forwardRef((props, ref) => <div ref={ref} id="outer" {...props} />)} />, container);
  ...
  describe('props validation', () => {
    ...
    it('should fail if non-numeric columnWidth is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} columnWidth="abc" />)).toThrow('An invalid "columnWidth" prop has been specified. ' + 'Value should be a number. ' + '"string" was specified.');
    });
    it('should fail if non-numeric rowHeight is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} rowHeight="abc" />)).toThrow('An invalid "rowHeight" prop has been specified. ' + 'Value should be a number. ' + '"string" was specified.');
    });
    it('should fail if no children value is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} children={undefined} />)).toThrow('An invalid "children" prop has been specified. ' + 'Value should be a React component. ' + '"undefined" was specified.');
    });
    it('should fail if an invalid direction is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} direction={null} />)).toThrow('An invalid "direction" prop has been specified. ' + 'Value should be either "ltr" or "rtl". ' + '"null" was specified.');
    });
    it('should fail if a string height is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} height="100%" />)).toThrow('An invalid "height" prop has been specified. ' + 'Grids must specify a number for height. ' + '"string" was specified.');
    });
    it('should fail if a string width is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeGrid {...defaultProps} width="100%" />)).toThrow('An invalid "width" prop has been specified. ' + 'Grids must specify a number for width. ' + '"string" was specified.');
    });
  });
});