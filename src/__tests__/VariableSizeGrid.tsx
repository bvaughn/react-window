...
describe('VariableSizeGrid', () => {
  let columnWidth, defaultProps, getScrollbarSize, itemRenderer, onItemsRendered, rowHeight; // Use PureComponent to test memoization.
  ...
  describe('estimatedColumnWidth and estimatedRowHeight', () => {
    it('should estimate an initial scrollable size based on this value', () => {
      ...
      const rendered = ReactTestRenderer.create(<VariableSizeGrid {...defaultProps} columnCount={50} columnWidth={columnWidth} estimatedColumnWidth={200} estimatedRowHeight={100} overscanColumnCount={0} overscanRowCount={0} rowCount={50} rowHeight={rowHeight} />);
  ...
  describe('resetAfterIndex method', () => {
    ...
    it('should delay the recalculation of the estimated total size if shouldForceUpdate is false', () => {
      const rendered = ReactTestRenderer.create(<VariableSizeGrid {...defaultProps} estimatedColumnWidth={30} estimatedRowHeight={30} overscanColumnCount={1} overscanRowCount={1} columnWidth={index => 50} rowHeight={index => 25} />);
      const scrollContainer = findScrollContainer(rendered);
      ...
      rendered.update(<VariableSizeGrid {...defaultProps} estimatedColumnWidth={30} estimatedRowHeight={30} overscanColumnCount={1} overscanRowCount={1} columnWidth={index => 40} rowHeight={index => 20} />);
      expect(scrollContainer.props.style.height).toEqual(575);
      expect(scrollContainer.props.style.width).toEqual(400);
  ...
  describe('props validation', () => {
    ...
    it('should fail if non-function columnWidth is provided', () => {
      expect(() => ReactTestRenderer.create(<VariableSizeGrid {...defaultProps} columnWidth={123} />)).toThrow('An invalid "columnWidth" prop has been specified. ' + 'Value should be a function. "number" was specified.');
    });
    it('should fail if non-function rowHeight is provided', () => {
      expect(() => ReactTestRenderer.create(<VariableSizeGrid {...defaultProps} rowHeight={123} />)).toThrow('An invalid "rowHeight" prop has been specified. ' + 'Value should be a function. "number" was specified.');
    });
  });
  describe('onScroll', () => {
    it('scrolling should report partial items correctly in onItemsRendered', () => {
      ...
      const instance = render(<VariableSizeGrid {...defaultProps} columnCount={100} columnWidth={() => 100} initialScrollLeft={20} initialScrollTop={10} rowCount={100} rowHeight={() => 25} />, document.createElement('div'));
      // grid 200w x 100h
  ...
  it('should descrease scroll size when itemCount decreases', () => {
    ...
    simulateScroll(gridRef.current, {
      scrollLeft: 3000,
      scrollTop: 4000
    });
