...
describe('FixedSizeList', () => {
  ...
  let mockedScrollWidth = Number.MAX_SAFE_INTEGER; // Use PureComponent to test memoization.
  ...
  describe('style overrides', () => {
    ...
    it('should support style prop', () => {
      const rendered = ReactTestRenderer.create(<FixedSizeList {...defaultProps} style={{
        backgroundColor: 'red'
      }} />);
  ...
  describe('scrollToItem method', () => {
    ...
    it('scroll with align = "auto" should work with very small lists and partial items', () => {
      const rendered = ReactTestRenderer.create( // Create list with only two items, one of which will be shown as a partial.
    ...
    it('should account for scrollbar size', () => {
      ...
      const ref = createRef();
      ReactDOM.render(<FixedSizeList {...defaultProps} ref={ref} />, document.createElement('div'));
  ...
  describe('itemKey', () => {
    ...
    it('should allow items to be moved within the collection without causing caching problems', () => {
      ...
      const rendered = ReactTestRenderer.create(<FixedSizeList {...defaultProps} itemCount={3} itemKey={itemKey}>
          {ItemRenderer}
        </FixedSizeList>);
      expect(itemKey).toHaveBeenCalledTimes(3);
      itemKey.mockClear();
  ...
  describe('custom element types', () => {
    ...
    it('should support spreading additional, arbitrary props, e.g. id', () => {
      ...
      ReactDOM.render(<FixedSizeList {...defaultProps} innerElementType={forwardRef((props, ref) => <div ref={ref} id="inner" {...props} />)} outerElementType={forwardRef((props, ref) => <div ref={ref} id="outer" {...props} />)} />, container);
    ...
    it('should warn if legacy direction "horizontal" value is used', () => {
      ...
      expect(console.warn).toHaveBeenCalledTimes(1);
      expect(console.warn).toHaveBeenLastCalledWith('The direction prop should be either "ltr" (default) or "rtl". ' + 'Please use the layout prop to specify "vertical" (default) or "horizontal" orientation.');
  ...
  describe('props validation', () => {
    ...
    it('should fail if non-numeric itemSize is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeList {...defaultProps} itemSize="abc" />)).toThrow('An invalid "itemSize" prop has been specified. ' + 'Value should be a number. ' + '"string" was specified.');
    });
    it('should fail if no children value is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeList {...defaultProps} children={undefined} />)).toThrow('An invalid "children" prop has been specified. ' + 'Value should be a React component. ' + '"undefined" was specified.');
    });
    it('should fail if an invalid layout is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeList {...defaultProps} layout={null} />)).toThrow('An invalid "layout" prop has been specified. ' + 'Value should be either "horizontal" or "vertical". ' + '"null" was specified.');
    });
    it('should fail if an invalid direction is provided', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeList {...defaultProps} direction={null} />)).toThrow('An invalid "direction" prop has been specified. ' + 'Value should be either "ltr" or "rtl". ' + '"null" was specified.');
    });
    it('should fail if a string height is provided for a vertical list', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeList {...defaultProps} layout="vertical" height="100%" />)).toThrow('An invalid "height" prop has been specified. ' + 'Vertical lists must specify a number for height. ' + '"string" was specified.');
    });
    it('should fail if a string width is provided for a horizontal list', () => {
      expect(() => ReactTestRenderer.create(<FixedSizeList {...defaultProps} layout="horizontal" width="100%" />)).toThrow('An invalid "width" prop has been specified. ' + 'Horizontal lists must specify a number for width. ' + '"string" was specified.');
    });
  });
});